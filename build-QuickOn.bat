...
set "output_name=QuickOn-init.exe"
...
rmdir /s /q %build_folder%\QuickOn

rem Create build and release folders if needed.
if not exist "%build_folder%\QuickOn" md "%build_folder%\QuickOn"
if not exist "%release_folder%" md "%release_folder%"

rem Make a copy of the file for easy compression later.
xcopy /i /e "%input_folder%data" "%build_folder%\QuickOn\data\"
xcopy /i /e "%input_folder%source" "%build_folder%\QuickOn\source\"
xcopy "%input_folder%LiesMich.txt" "%build_folder%\QuickOn\"
xcopy "%input_folder%ReadMe.txt"  "%build_folder%\QuickOn\"

rem Compile QuickOn-init.exe
"%aut2exe%" /in "%build_folder%\QuickOn\source\QuickOn.au3" /out "%build_folder%\QuickOn\QuickOn-init.exe" /icon "%build_folder%\QuickOn\source\QuickOn.ico" /x86
if not exist "%build_folder%\QuickOn\QuickOn-init.exe" (
...
IF "%~1"=="-s" (
	...
	"%signtool%" sign /a "%build_folder%\QuickOn\QuickOn-init.exe"
)

rem Make a release by packing the exe, data and source code into a self-extracting archive.
pushd %build_folder%
"%sevenzip%" a -r -x!.git -sfx7z.sfx "%release_folder%\QuickOn.tmp" "QuickOn"
popd

rem Change the icon on the self-extracting archive.
"%reshack%" -addoverwrite "%release_folder%\QuickOn.tmp", "%release_folder%\%output_name%", "%build_folder%\QuickOn\source\QuickOn.ico",ICONGROUP,1,1033

del /q "%release_folder%\QuickOn.tmp"
